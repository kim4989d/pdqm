...
public class EgovBBSAttributeManageController {
	...
	// khyoon 게시물 분류 공통코드 선택기능 추가 2011.10.26
	ComCodeVO clvo = new ComCodeVO();
	clvo.setClCode("PQC");	// 공공기관보유정보품질진단개선 공통코드 서비스
	codeResult = cmmUseService.selectCmmCode(clvo);
	model.addAttribute("clList", codeResult);
	//~khyoon
	...
	*/
	//if (isAuthenticated) {
		...
		boardMaster.setFrstRegisterId("khyoon");
	    boardMaster.setUseAt("Y");
	    boardMaster.setTrgetId("SYSTEMDEFAULT_REGIST");
	    
		// khyoon 2011.11.16 게시판 소개 HTML Decoding 처리
		String bbsIntrcn = boardMaster.getBbsIntrcn();
		boardMaster.setBbsIntrcn(bbsIntrcnToHTMLDecode(bbsIntrcn));
		//~khyoon	    
	...
	//if (isAuthenticated) {
    	...
    	boardMaster.setLastUpdusrId("khyoon");
    ...
    // khyoon 2011.11.16
    private String bbsIntrcnToHTMLDecode(String str)
    {
    	String strHTML;
    	strHTML = str.replace("&lt;","<");
    	...
    	strHTML = strHTML.replace("&quot;","\"");
    	strHTML = strHTML.replace("&apos;","'");
    	strHTML = strHTML.replace("<c:url value='","/pdqm");
    	strHTML = strHTML.replace("'/>",""); 
