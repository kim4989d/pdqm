...
public class EgovCmmUseServiceImpl extends AbstractServiceImpl implements EgovCmmUseService {
	...
	return cmmUseDAO.selectCmmCode(vo);
    }    
    
    /**
     * khyoon ComCodeVO의 리스트를 받아서 여러개의 코드 리스트를 맵에 담아서 리턴한다. 2011.11.08
     * 
     * @param voList
     * @return
     * @throws Exception
	...
	ComCodeVO vo;
	Map<String, List<CmmnCode>> map = new HashMap<String, List<CmmnCode>>();

	Iterator iter = voList.iterator();
	while (iter.hasNext()) {
	    vo = (ComCodeVO)iter.next();
	    map.put(vo.getCodeId(), cmmUseDAO.selectCmmCode(vo));
	}
