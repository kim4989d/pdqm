...
import egovframework.com.uss.ion.bnr.service.MainVO;
...
public class EgovComIndexController implements ApplicationContextAware ,InitializingBean {
	...
	public String index(HttpServletRequest request, HttpServletResponse response, ModelMap model){
		...
		// 배너관리중 지식서비스 가져오기.
		try {
			bannerVO.setBannerType("2");
			bannerVO.setReflctAt("Y");
			bannerVO.setRowNum("9");
			bannerVO.setBannerList(egovBannerService.selectKnowledgeList(bannerVO));

		} catch (Exception ex) {
			// TODO Auto-generated catch block
			log.debug("knowledgeList: " + ex.getMessage());
		}
		model.addAttribute("knowledgeList", bannerVO.getBannerList());
		
		// 배너관리중 팝업존 가져오기
		try {
			bannerVO.setBannerType("1");
			bannerVO.setReflctAt("Y");
			bannerVO.setRowNum("4");
			bannerVO.setBannerList(egovBannerService.selectKnowledgeList(bannerVO));

		} catch (Exception ex) {
			// TODO Auto-generated catch block
			log.debug("popupList: " + ex.getMessage());
		}
		model.addAttribute("popupList", bannerVO.getBannerList());
		
		// 공지사항 가져오기 bbsId = 받아야 함.
		MainVO mainVO = new MainVO();
		mainVO.setBbsId("BBSMSTR_000000000121");
		mainVO.setUseAt("Y");

		List<MainVO> result = null;
		try {

			result = egovBannerService.selectNoticeList(mainVO);
		} catch (Exception ex) {
			log.debug("noticeList: " + ex.getMessage());
		}
		model.addAttribute("noticeList", result);
		
		// 보도자료 가져오기
		mainVO.setBbsId("BBSMSTR_000000000191");
		mainVO.setUseAt("Y");
		try {

			result = egovBannerService.selectNoticeList(mainVO);
		} catch (Exception ex) {
			log.debug("reportList: " + ex.getMessage());
		}
		...
		// 서버체크
		if(StrServerName.equals("localhost")) {
			model.addAttribute("serverName", StrServerName);
		} else {
			model.addAttribute("serverName", StrServerName);
		}
		return "egovframework/pdqm/index";
	...
	public String AdminMain(){
		return "egovframework/pdqm/admin/AdminMain";
	}

	@RequestMapping("/AdminTop.do")
	public String Admintop(){
		return "egovframework/pdqm/admin/AdminTop";
	}

	@RequestMapping("/AdminLeft.do")
	public String AdminLeft(){
		return "egovframework/pdqm/admin/AdminLeft";
	}

	@RequestMapping("/AdminContent.do")
	public String AdminContent(ModelMap model){

		LoginVO loginVO = (LoginVO)EgovUserDetailsHelper.getAuthenticatedUser();
		model.addAttribute("loginVO", loginVO);

		return "egovframework/pdqm/admin/AdminContent";
	}
