...
public class EgovMberManageServiceImpl extends AbstractServiceImpl implements EgovMberManageService {
	...
	public void mberSelectUpdt(MberManageVO mberManageVO) throws Exception {
		//패스워드 암호화
		String pass = EgovFileScrty.encryptPassword(mberManageVO.getPassword());
		mberManageVO.setPassword(pass);
		
    	System.out.println("getMberEmailAdres=========>"+mberManageVO.getMberEmailAdres());
    	System.out.println("getCompanyName=========>"+mberManageVO.getCompanyName());
    	System.out.println("getDepartmentName=========>"+mberManageVO.getDepartmentName());
    	System.out.println("getEndTelno=========>"+mberManageVO.getEndTelno());
    	System.out.println("Password=========>"+mberManageVO.getPassword());
    	System.out.println("UniqId=========>"+mberManageVO.getUniqId());
		
		
		mberManageDAO.mberSelectUpdt(mberManageVO);
	}
	
	
	public void updateMberGrp(String checkedId, String mberGrp) throws Exception {
		//회원그룹 update
		String [] delId = checkedId.split(",");
		for (int i=0; i<delId.length ; i++){
			String [] id = delId[i].split(":");
			if (id[0].equals("USR01")){
		        //적용
				mberManageDAO.updateMberGrp(id[1], mberGrp);
			}
		}		
	...
	public void updateMberQuit(String uniqId) throws Exception {
		//회원탈퇴 update

		mberManageDAO.updateMberQuit(uniqId);
	}
	...
	public MberManageVO selectEmail(MberManageVO mberManageVO) {
		return mberManageDAO.selectEmail(mberManageVO);
	}	
}