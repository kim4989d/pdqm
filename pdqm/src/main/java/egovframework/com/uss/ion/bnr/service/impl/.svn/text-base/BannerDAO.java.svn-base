...
public class BannerDAO extends EgovComAbstractDAO {
	...
	/**
	 * 배너를 관리하기 지식서비스에 등록된 배너목록을 조회한다.
	 * @param bannerVO - 배너 Vo
	 * @return List - 배너 목록
	 * @exception Exception
	 */
	@SuppressWarnings("unchecked")
	public List<BannerVO> selectKnowledgeList(BannerVO bannerVO) throws Exception {
		return list("bannerDAO.selectKnowledgeList", bannerVO);
	}

	/**
	 * 공지사항를 관리하기 위해 등록된 메인목록을 조회한다.
	 * @param bannerVO - 공지사항 Vo
	 * @return List - 공지사항 목록
	 * @exception Exception
	 */
	@SuppressWarnings("unchecked")
	public List<MainVO> selectNoticeList(MainVO mainVO) throws Exception {
		return list("bannerDAO.selectNoticeList", mainVO);
	}	
    /**
	 * 배너목록 총 갯수를 조회한다.
