...
function fn_update(){
	...
	if (fn_fomrValidate()){
		if(confirm("<spring:message code='common.update.msg'/>")){
			document.form.action = "<c:url value='/support/diagnosis/level/updateScoreViewPoint.do'/>";
 ...
 ******************************************************** */
function fn_list(){
	location.href = "<c:url value='/support/diagnosis/level/selectScoreViewPointList.do'/>";
}
...
<!-- contents: Start -->
	<div class="title">
		<div class="locationMap">품질수준 자가진단관리 > 관리수준 자가진단 > 결과(진단관점) 점수 관리</div>
		<h2>결과(진단관점) 점수 수정</h2>
	</div>
	...
	<fieldset>
		...
		<table class="searchType1" summary="검색조건선택 테이블입니다.">
			...
			<tr>
				...
				<td>
					<form:input path="upperDgnssTrgetNm" cssClass="readonly" cssStyle="width:150px" Disabled ="true "  />
	...
	<table class="tblType1 tblSpace" summary="목록 테이블입니다.">
		...
		<tbody>
			<c:forEach items="${resultList}" var="resultList" varStatus="status">
				<tr>
					<td><input type="text" id="mumm_${status.count}" name="mumm_${status.count}" value="${resultList.mumm}" style="width:100px"vonblur="onlyNum(this)"  maxlength="3" class="text" title="최소값 입력"/></td>
					<td>&lt;=</td>
