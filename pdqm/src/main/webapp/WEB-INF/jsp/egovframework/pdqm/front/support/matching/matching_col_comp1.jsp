...
//		</c:forEach>
		function fn_linkPage(pageNo){
			//alert("Fn_lnkPage[" + pageNo + "]");
			/*location.href = "/support/matching/searchTBLInfoList.do?pageNo="+pageNo;*/
			document.matchingCompForm.pageIndex.value = pageNo;
			document.matchingCompForm.action = "<c:url value='/support/matching/compCol1List.do'/>";
		...
		function fn_view_compDtlRtn(rn, comp_rtn, column_id) {
			...
			if (comp_rtn>0){
				...
				document.matchingCompForm.action = "<c:url value='/support/matching/viewComp1DtlRtn.do'/>";
			   	document.matchingCompForm.submit();
			}else{
				alert("불일치 결과가 없습니다.");
			}
		...
		function fn_excel_compRtn(rn) {
			document.matchingCompForm.rowNo.value = rn;
			document.matchingCompForm.action = "<c:url value='/support/matching/excelComp1Rtn.do'/>";
		   	document.matchingCompForm.submit();
		}
	</script>
</head>
...
<div class="bg_sup">
	...
	<div id="conBox">
		...
		<table class="conTable1 brN" summary="컬럼비교 게시판 입니다">
		...
		<table class="conTable1 brN mt_10" summary="컬럼비교 게시판 입니다">
			...
			<tbody>
				<c:forEach var="colInfoVO" items="${compCol1Rtnlist}">
					<tr class="ta_1">
						<td class="ta_c">
							${colInfoVO.rn}
