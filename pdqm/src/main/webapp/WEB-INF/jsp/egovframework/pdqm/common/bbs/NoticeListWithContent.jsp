...
<%
 /**
  * @Class Name : NoticeListWithContent.jsp
  * @Description : 게시물 목록과 내용함께보기 화면
...
<!--// 검색박스 -->
	...
	<dl class="tbType2">
		...
		<c:if test="${brdMstrVO.snsExportPosblAt == 'Y' }">
			...
			<a href="http://me2day.net" target="_blank"
		...
		<dd class="con">
			<div id="con1">
				...
				<p class="blog">
					...
					<a href="${contentVO.blogUrl}" target="_blank">${contentVO.blogUrl}</a>
					</c:if>

					<img src="<c:url value='/images/pdqm/common/bbs/txt_sns.gif'/>" class="mal_10" alt="SNS바로가기" />
					<c:if test="${contentVO.facebookUrl != null }">
					<a href="${contentVO.facebookUrl}" target="_blank"><img src="<c:url value='/images/pdqm/common/icon/icon_facebook2.gif'/>" alt="전문가의 페이스북 바로가기" /></a>
					</c:if>
					<c:if test="${contentVO.twitterUrl != null }">
					<a href="${contentVO.twitterUrl}" target="_blank"><img src="<c:url value='/images/pdqm/common/icon/icon_twitter2.gif'/>" alt="전문가의 트위터 바로가기" /></a>
					</c:if>
				</p>
				</c:if>
