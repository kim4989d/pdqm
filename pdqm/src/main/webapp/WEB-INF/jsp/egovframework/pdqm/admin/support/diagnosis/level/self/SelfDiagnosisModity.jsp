...
<%
/**
 * @Class Name : SelfDiagnosisModity.jsp
  ...
  function fn_update(){
		if (fn_fomrValidate()){
			if(confirm("<spring:message code='common.update.msg'/>")){
				document.form.action = "<c:url value='/support/diagnosis/level/updateSelfDiagnosis.do' />";
...
function fn_delete(){
	if (document.form.resultCo.value > 0){
		alert("응답자가 있는 자가진단은 삭제할 수 없습니다.");
		return;
	}

	if(confirm("<spring:message code='common.delete.msg'/>")){
		document.form.action = "<c:url value='/support/diagnosis/level/deleteSelfDiagnosis.do' />";
	   	document.form.submit();
		...
		<table class="vblType1" summary="관리수준 자가진단 등록 테이블입니다.">
			...
			<tr>
				...
				<td>
					<form:hidden  path="ntceBeginDt" cssClass="text" cssStyle="width:100px"   maxlength="50" />
					  <fmt:parseDate value="${diagnosisLevelSelf.ntceBeginDt}" var="dateFmt" pattern="yyyymmdd"/>
			...
			<tr>
				...
				<td>
					...
					<fmt:parseDate value="${diagnosisLevelSelf.ntceEndDt}" var="dateFmt" pattern="yyyymmdd"/>
