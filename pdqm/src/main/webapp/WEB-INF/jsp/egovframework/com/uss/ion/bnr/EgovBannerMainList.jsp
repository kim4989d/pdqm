 ...
 <c:forEach var="banner" items="${bannerList}" varStatus="status">
  <tr>
    <td class="lt_text6" nowrap>
            <span class="link">
                <!-- psh <input type="submit" name="bannerNm" id="bannerNm" value="<c:out value="${banner.bannerNm}"/>" onclick="fncSelectBanner('<c:out value="${banner.bannerId}"/>');return false;"/> -->
				<a href="<c:out value="${banner.linkUrl}"/>"><img src="C:\egovframework\upload\<c:out value="${banner.bannerImage}"/>" alt="<c:out value="${banner.bannerDc}"/>" /></a>
            </span>
    </td>
  </tr>
 </c:forEach>
