...
<table width="100%" cellpadding="8" class="table-search" border="0">
 <tr>
  <td width="40%" class="title_left">
  	<h1>
  		<img src="<c:url value='/images/egovframework/com/cmm/icon/tit_icon.gif' />" width="16" height="16" hspace="3" align="middle" alt="">&nbsp;<c:out value="${brdMstrVO.bbsNm}"/>
  	</h1>
  </td>
  </tr>
  <tr>
  <td width="10%"class="title_left">
  	...
  	<c:if test="${brdMstrVO.nttClPosblAt == 'Y' }">
	    <form:select name="nttClCode" path="nttClCode" title="게시물분류코드 선택">
	  		<form:option value='' label="--선택하세요--" />
	   		<form:options items="${codeList}" itemValue="code" itemLabel="codeNm"/>
		</form:select>
		<form:errors path="nttClCode" />  		
  	</c:if>
  	<!-- ~khyoon -->
...
<table width="100%" cellpadding="8" class="listTable" summary="번호, 제목, 게시시작일, 게시종료일, 작성자, 작성일, 조회수   입니다">
 <thead>
  <tr>
    ...
    <th scope="col" class="listTitle" width="5%" nowrap>No</th>
    <!-- khyoon 게시물분류가능 게시판인 경우 분류 컬럼 활성화 2011.10.28 -->
  	<c:if test="${brdMstrVO.nttClPosblAt == 'Y' }">
  		<th scope="col" class="listTitle" width="20%" nowrap>분류</th>
  	</c:if>    
    <th scope="col" class="listTitle" width="44%" nowrap>제목</th>
   	...
   	<c:if test="${anonymous != 'true'}">
    	<th scope="col" class="listTitle" width="10%" nowrap>작성자</th>
    </c:if>
    <th scope="col" class="listTitle" width="12%" nowrap>작성일</th>
    <!-- khyoon  -->
	<c:if test="${brdMstrVO.fileAtchPosblAt == 'Y'}">
		<th scope="col" class="listTitle" width="8%" nowrap>첨부</th>
	</c:if>
    <!--~khyoon  -->
 ...
 <tbody>
	 <c:forEach var="result" items="${resultList}" varStatus="status">
	  <tr>
	    ...
	    <!-- khyoon 첨부파일 확장자별 아이콘 이미지 표시 기능 추가 2011.10.24 -->
	    <c:if test="${brdMstrVO.fileAtchPosblAt == 'Y'}">
	    	<td class="listCenter" nowrap>
	    		<c:if test="${result.fileExtsn != null }">
    				<img src="<c:url value='/images/egovframework/com/cmm/icon/icon_${result.fileExtsn}.gif'/>">
    			</c:if>
	    	</td>
	    </c:if>
