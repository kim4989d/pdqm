...
var editor = null;
// khyoon 2011.11.23 툴바 숨기고 편집기 초기화
function initEditorHideToolbar() {
...
cfg.registerButton("my-sample", "Class: sample", "ed_custom.gif", false,
  ...
  */
  cfg.pageStyle = "body { background-color: } .hilite { background-color: yellow; } "+
...
HTMLArea.Config = function () {
	...
	// khyoon 2011.11.17 편집기 초기 스타일 설정
	this.pageStyle = "body { FONT-SIZE: 10pt; FONT-FAMILY: 돋움; LINE-HEIGHT: 1.4; }"; 
	//~khyoon
...
HTMLArea.prototype._editorEvent = function(ev) {
	...
	if (keyEvent && ev.ctrlKey && !ev.altKey) {
		...
		switch (ev.keyCode) {
		    case 13: // KEY enter
		    // khyoon 2011.12.09 ENTER double space BUG 수정
...
HTMLArea.prototype.dom_checkInsertP = function() {
	...
	if (!block) {
		...
		if (left != body && right != body) {
			...
			r2.surroundContents(block = doc.createElement("<br />"));
			if (!/\S/.test(HTMLArea.getInnerText(block)))
				block.innerHTML = "<br />";
			block.normalize();
			r.setEndAfter(right);
			r.surroundContents(block = doc.createElement("<br />"));
			if (!/\S/.test(HTMLArea.getInnerText(block)))
				block.innerHTML = "<br />";
			block.normalize();
		} else {
			...
			r.setStart(body, 0);
			r.setEnd(body, 0);
			r.insertNode(block = doc.createElement("<br />"));
			block.innerHTML = "<br />";
		}
